...
class check_ping_core_class(aetest.Testcase):
    ...
    def setup(self, testbed, device_name, targets):
        ...
        ping_destinations = targets.get(device_name, [])

        for dest in ping_destinations:
            logger.info(banner(f'Ping to {dest}'))
            # parser = Ping(device=device)
            # parsed = parser.parse(addr=dest)
            parsed = device.parse('ping', addr=dest)
    ...
    def check_ping(self, steps, device_name, targets):
        ...
        ping_destinations = targets[device_name]

        for dest in ping_destinations:
            # この宛先に関してのステップ
            with steps.start(dest, continue_=True) as dest_step:
                parsed = self.ping_dict[dest]
